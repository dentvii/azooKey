...
public struct TabCharacterPreference: Codable, StaticInitialValueAvailable {
    ...
    mutating func setUsed(base: String, for tab: TabIdentifier) {
        ...
        if let index = items.firstIndex(where: {$0.base == base}) {
            items[index].history.append(Date())
        } else {
            let newItem = RecentlyUsedItem(base: base, history: [Date()])
    ...
    public mutating func setPreference(base: String, replace: String, for tab: TabIdentifier) {
        if let index = self.preferences.firstIndex(where: {$0.tab == tab && $0.base == base}) {
            self.preferences[index].replace = replace
            self.preferences[index].lastUpdate = Date()
        } else {
            self.preferences.append(.init(tab: tab, base: base, replace: replace, lastUpdate: Date()))
        }
    }
